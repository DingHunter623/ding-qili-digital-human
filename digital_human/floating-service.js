...
const HERO_EYEBROW='制造改善方法论 / 项目实践展示 / 精益顾问交流入口';
...
function applyBranding(){
  ...
  const eyebrow=document.querySelector('.hero .eyebrow'); if(eyebrow)eyebrow.textContent=HERO_EYEBROW;
  const primary=document.querySelector('.actions .button.primary'); if(primary&&/数字人|问答/.test(primary.textContent))primary.textContent=MAIN_CTA;
  const ans=document.getElementById('answer');
  if(ans&&ans.textContent){ans.textContent=ans.textContent.replace(/丁启利职业能力AI数字人/g,'丁启利制造改善方法论与项目实践交流助手').replace(/AI数字人/g,'专业交流助手');}
}
function enhanceSkillTags(){
  const tags=document.querySelector('.hero .tags'); if(!tags)return;
  if(!Array.from(tags.children).some(li=>li.textContent.trim()==='新工厂规划')){const li=document.createElement('li');li.textContent='新工厂规划';tags.appendChild(li);}
}
function enhanceTopicModules(){
  const wrap=document.querySelector('.topic-controls'); if(!wrap)return;
  try{
    if(typeof answers!=='undefined'&&answers.intro){answers.intro=answers.intro.replace(/您好，\s*我是丁启利职业能力AI数字人。/,'您好，我是丁启利制造改善方法论与项目实践交流助手。').replace(/AI数字人/g,'专业交流助手');}
  }catch(e){}
  function addTopic(topic,label,questionText,answerText,afterTopic){
    ...
    btn.addEventListener('click',()=>{try{setTopic(topic)}catch(e){const q=document.getElementById('question'),a=document.getElementById('answer');if(q)q.value=questionText;if(a)a.textContent=answerText;}});
  ...
  if(q&&!q.dataset.topicEnhanced){q.dataset.topicEnhanced='true';q.addEventListener('input',()=>{const v=q.value;if(/OEE|稼动率|设备效率|可用率|性能效率|良品率|停机/i.test(v)){try{setTopic('oee',false)}catch(e){}}else if(/新工厂|厂房规划|工厂规划|Layout|布局|物流动线|产线规划|园区规划|车间规划/i.test(v)){try{setTopic('factory',false)}catch(e){}}else if(/目视化|可视化|标识|标牌|看板|6S|通道|库位|点位|现场管理语言/i.test(v)){try{setTopic('visual',false)}catch(e){}}});}
}
